...
// console.log(getName(makeDirectory("foobar", [])));
// console.log(getName(makeFile("foobar")));
...
export function getContentNames(dir: Directory): string[] {
  // TODO: implement me
  let arr:string[] =[];
  const fstr3:FileSystemTree[] = dir.contents;
     for ( let i =0; fstr3.length > i; i++){
         arr.push(fstr3[i].name);
     }
  return arr;
}

// console.log(getContentNames(makeDirectory("foobar", [
//   makeFile("squid-game-1.mpeg")
// ])));
// console.log(getContentNames(makeDirectory("/", [
//   makeFile("squid-game-1.mpeg"),
//   makeFile("squid-game-2.mpeg")
// ])));
/* ----------------------------------------------------- **
...
// console.log( findFileOrDirectory(makeDirectory("foobar", [
//   makeFile("squid-game-1.mpeg")
// ]), "squid-game-2.mpeg") );
...
export function replaceEntry(fstr: FileSystemTree[], newEntry: FileSystemTree): FileSystemTree[] {
  // TODO: implement me
  const newEntries:FileSystemTree[]=[];
  for ( let i =0; i<fstr.length; i++){
     if ( newEntry.name === fstr[i].name && fstr[i].tag === newEntry.tag){
       newEntries.push(newEntry);
     } else if( newEntry.name ===fstr[i].name && fstr[i].tag !== newEntry.tag){
       newEntries.push(newEntry);
     } else{
       newEntries.push(fstr[i]);
     }     
  }  
  return newEntries;  
}
// const xs = [makeFile("squid-game-1.mpeg")];
// console.log(replaceEntry(xs, makeFile("squid-game-1.mpeg")));
// console.log(replaceEntry(xs, makeDirectory("squid-game-1.mpeg", [])));
...
// console.log( replaceEntry([dir1, makeFile("squid-game-3.mpeg")], dir2));
...
export function createFileOrDirInDir(dir: Directory, tag: "FILE" | "DIRECTORY", name: string): Directory {
  ...
  let newDir = dir;
  for ( let i = 0; i<newDir.contents.length; i++){
      if ( newDir.contents[i].name === name && newDir.contents[i].tag === tag){  
          return dir;      
      }
    }
    if ( tag === "FILE"){
    newDir.contents.push(makeFile(name));
      }
    else if( tag === "DIRECTORY"){
    newDir.contents.push(makeDirectory(name,[]));
     } 
  return newDir;
}
// const dir3 = makeDirectory("CSC-600",[makeFile("squid"),makeFile("seaweed"),makeDirectory("hw4",[])]);

// console.log(fileSystemTreeToString(createFileOrDirInDir(dir3, "FILE", "hw4.td")) );
// console.log(fileSystemTreeToString(createFileOrDirInDir(dir3, "DIRECTORY", "hw4")) );
...
export function copyFileSystemTree(fstr: FileSystemTree): FileSystemTree {
  ...
  // TODO: implement me
  let fstr1 = makeDirectory(fstr.name,[]);
  const helper= (fstr2:FileSystemTree, dir?:Directory)=>{
    if(!dir){
      dir = fstr1;
    }
    switch(fstr2.tag){
      case "DIRECTORY":{
        let tempDir = makeDirectory(fstr2.name, []);
              dir.contents.push(tempDir);
              for(let content of fstr2.contents){
                  helper(content, tempDir);
              } 
          break;
      }
      case "FILE":{
        dir.contents.push(makeFile(fstr2.name));
          break;
      }
    }
  }
   helper(fstr);
   return fstr1.contents[0];
}
// console.log(fileSystemTreeToString(copyFileSystemTree(fstr1)));
...
export function createFileOrDir(fstr: FileSystemTree, path: string[], tag: "FILE" | "DIRECTORY", name: string): FileSystemTree {
  ...
  const helper =( fstr1: FileSystemTree, path1: string[], tag1: "FILE" | "DIRECTORY", name1: string)=> {
    let dirContents:FileSystemTree[]=[];
         switch( path.length){
           case 0:{
             return fstr1;
           }
           case 1:{
             switch(fstr1.tag){
               ...
               case "DIRECTORY":{
                 if ( fstr1.name === path1[0]){
                 return createFileOrDirInDir(fstr1,tag1,name1);
                 }
                 else {
                   return fstr1;
                 }
         ...
         if( path[0]=== fstr1.name){
           switch(fstr1.tag){
             case "FILE":{
               return fstr1;
             }
             case "DIRECTORY":{
               for ( let i =0; i<fstr1.contents.length; i++){
                 const fstrContainer = createFileOrDir(fstr1.contents[i],path.slice(1),tag1,name1);
                 dirContents.push(fstrContainer);
               }
               let newFstr = makeDirectory(fstr1.name,dirContents);
               return newFstr;
             }
           }
    ...
    return helper(fstr,path,tag,name);  
   }
// console.log("----"); 
// const fstr2 = createFileOrDir(fstr1, ["/"], "FILE", "tmp.txt");
// console.log(fileSystemTreeToString(fstr2));
// const fstr3 = createFileOrDir(fstr2, ["/"], "FILE", "tmp2.txt");
// console.log(fileSystemTreeToString(fstr3));

// const fstr4 = createFileOrDir(fstr3, ["/"], "FILE", "tmp2.txt");
// console.log(fileSystemTreeToString(fstr4));
...
export function findFilesOrDirs(fstr: FileSystemTree, predicate: (name: string) => boolean): string[][] {
  ...
  const helper =(fstr1: FileSystemTree, path: string[] = []) =>{
      path = [...path, fstr1.name];
      switch(fstr1.tag){
          case 'DIRECTORY':
              for(let content of fstr1.contents){
                  if(predicate(fstr1.name)){
                  paths.push(path);
              }
                  helper(content, path);
              }
...
// console.log(findFilesOrDirs(fstr1, (name: string) => name.endsWith(".ts")));
// console.log(findFilesOrDirs(fstr1, (name: string) => name.endsWith(".app")));