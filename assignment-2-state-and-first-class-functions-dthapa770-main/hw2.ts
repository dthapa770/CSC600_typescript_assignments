...
export function f4<T, U, V>(arg1: T,
                             arg2: U, 
                             arg3: (x: T) => 
                             (y: U) => 
                             V): V  {
...
export function arrayOfArithmeticFunctions(names: ("plus"|"minus"|"times"|"divide")[]): ((x: number, y: number) => number)[] {
    ...
    let i= 0;
        while(i<names.length){
            ...
            switch (name) {
                case "plus": {
                    result.push((x: number, y: number) => x + y);
                    break;
                }
                case "minus": {
                    result.push((x: number, y: number) => x - y);
...
export function CBetterPair<S, T>(fst: S, snd: T): CBetterPair<S, T> {
    ...
    const getFst= ()=>{return first};
    const getSnd= ()=>{return second};
...
export function allPathsSatisfyingPredicate(predicate: (authority: string) => boolean,
   ...
   let arr:JSONValue[]=[];
    function helperFunction(predicate: (authority: string) => boolean, jValue: JSONValue){
    ...
    return arr.map((data:any) => data.path || '/' );
}
...
export function countPathsSatisfyingPredicate(predicate: (authority: string) => boolean,
    ...
    helperFunction(predicate,json);
    return arr.map((data:any) => data.paths|| '/').reduce( (previous:number, current:string) => {
...
export function allPaths(authority: string, json: JSONValue): string[] {
    return allPathsSatisfyingPredicate((auth: string) => auth === authority, json);
}

export function countPaths(authority: string, json: JSONValue): number {
    return countPathsSatisfyingPredicate((auth: string) => auth === authority, json);
}
...
export function fillInMissingPath(json: JSONValue): JSONValue {
    // TODO: implement me
    const changeEntries = (jValue: JSONValue): JSONValue => {
        if(!jValue || !Array.isArray(jValue)){
            return {};
        }
        jValue = jValue.map((data: any) => {
            if(data.links.length !== 0){
...
export function arrayToMerkleTree<T>(arr: T[]): MerkleTree<T> {
    // TODO: implement me
    let tree = [];
    let leaves = arr.map((data: T) => MLeaf(data, 0));
    ...
    while(tree[tree.length-1].length !== 1 || tree[tree.length-1][0].tag === "LEAF"){
        let currArr = tree[tree.length-1]; 
        let nodeArr:MerkleTree<T>[] = [];
        while(currArr.length > 0){
            let tuple = currArr.splice(0, 2);
            if(tuple.length === 0){
                continue;
            }
            let node:MerkleTree<T> = MNode(0, tuple[0], tuple[1]);
            nodeArr.push(node);
        }
        tree.push(nodeArr);
    }
...
export function hashFromLeafToRoot<T>(hashData: (x: T) => number,
    ...
    // TODO: implement me
    const checkNode = (node: MerkleTree<T>) => {
        if(!node){
            return;
        }
        if(node.tag === 'NODE'){
    ...
    const calcHash = (node: MerkleTree<T>) =>{
        switch(node.tag){
            case 'LEAF': {
            ...
            case 'NODE': {
                 let leftHash = hash(node.left.hashValue);
                 let rightHash = hash(node.right.hashValue);
