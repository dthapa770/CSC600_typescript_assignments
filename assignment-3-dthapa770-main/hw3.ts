...
export class Tree<T> {
    ...
    ** ----------------------------------------------------- */
    public map<U>(f: (arg: T) => U): Tree<U> {
        // TODO: implement me
        const newTree = (next: undefined|Tree<T>): Tree<U>|undefined => {
            if(!next) {
                return undefined;
            }
            let newNode = new Tree<U>(f(next.contents), newTree(next.left), newTree(next.right))
             return newNode;
         }
         return newTree(this) ?? new Tree(f(this.contents), undefined, undefined);
    }
}
...
export class Tree3<T> extends Tree<T> {
    ...
    ** ----------------------------------------------------- */
    public map<U>(f: (arg: T) => U): Tree<U> {
        // TODO: Implement me
        const newTree = (next: undefined | any): Tree3<U>|undefined => {
            if(!next) {
                return undefined;
            }
            let  newNode = new Tree3<U>(f(next.contents), newTree(next.left), newTree(next.middle), newTree(next.right));
            return newNode;
        }
        return newTree(this) ?? new Tree(f(this.contents), undefined,  undefined);
    }
    ...
    ** ----------------------------------------------------- */
    public toTree2(): Tree<T> {
        // TODO: Implement me
        const newTree = (next: undefined | any): Tree<T>|undefined => {
            if(!next) {
                return undefined;
            }
            let  newNode;
            if(next.middle){
                newNode = new Tree<T>(next.contents, new Tree<T>(next.middle.contents, newTree(next.left), undefined), newTree(next.right));
            }
            else{
                newNode = new Tree<T>(next.contents, newTree(next.left),  newTree(next.right));
            }
            return newNode;
        }
        return newTree(this) ?? new Tree(this.contents, undefined,  undefined);
    }
}
...
export class WebEntryJSONValue extends ObjJSONValue {
    ...
    public fillInMissingPath(): JSONValue {
        // TODO: implement me
        return new WebEntryPathJSONValue((<StringJSONValue>this.obj.authority).str, '/', <ArrJSONValue>new ArrJSONValue((<ArrJSONValue>this.obj.links).arr).fillInMissingPath());
    }
}
